...
const KIND_META: Record<SummaryKind, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
  total: {
    label: '전체 설비',
    color: '#003087',
    bg: '#eff6ff',
    ...
    async function load() {
      try {
        if (kind === 'alarm') {
          ...
          if (kind === 'total') { setValue(total); setSub(`전체 ${total}대`) }
          else if (kind === 'active') { setValue(active_); setSub(`가동률 ${total ? Math.round((active_ / total) * 100) : 0}%`) }
          else if (kind === 'inactive') { setValue(total - active_); setSub(`전체 ${total}대 중`) }
          else if (kind === 'comm-fault') { setValue(noData); setSub(`응답 없음`) }
        }
      } catch {
