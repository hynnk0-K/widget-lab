...
import { useProcessMap } from './model/useProcessMap'
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[480px] bg-red-50 rounded-xl border border-red-200">
        ...
        <button onClick={() => navigate('/service/process')} className="mt-3 h-8 px-4 text-[12px] text-red-600 underline">
          공정 목록으로
        </button>
      </div>
    )
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => navigate('/service/process')} className="text-[12px] text-slate-400 hover:text-slate-600">
            ← 공정 목록
          </button>
      ...
      {mode === 'image' ? (
        <LayoutMap
          ...
          onPinClick={(pinId) => navigate(`/service/line/${pinId}/map`)}
        />
      ) : (
        <>
          <DiagramMap
