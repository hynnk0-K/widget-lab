...
프로젝트 상시 규칙 요약은 [../CLAUDE.md](../CLAUDE.md)에 있다. 이 문서는 전문(全文)이며, 리뷰·리팩터링 시 참고한다.
...
> 이 저장소는 FSD(shared / entities / features / widgets / pages)로 이 원칙을 적용한다. CLAUDE.md의 아키텍처 절을 따른다.
...
function EmailField({ value, onChange, error }) {
  ...
  return (
    <input value={value} onChange={(e) => onChange(e.target.value, validate(e.target.value))} />
  )
}
```
...
> 이 중 파일/함수 길이·중첩 깊이는 `eslint.config.js`에서 경고로 자동 감지된다.
